...
from backend.models.schemas import (
    ...
    BusinessLogic, ValidationError, ValidationReport, ValidationStatus
)
...
def repair_schemas(
    ...
    if ui_errors:
        ...
        ui_schema, count = _repair_layer("UI Schema", json.dumps(ui_schema.model_dump(), indent=2), ui_errors, UISchema)
        fixed_count += count

    if api_errors:
        print(f"   → Repairing API Schema ({len(api_errors)} errors)...")
        api_schema, count = _repair_layer("API Schema", json.dumps(api_schema.model_dump(), indent=2), api_errors, APISchema)
        fixed_count += count

    if db_errors:
        print(f"   → Repairing DB Schema ({len(db_errors)} errors)...")
        db_schema, count = _repair_layer("DB Schema", json.dumps(db_schema.model_dump(), indent=2), db_errors, DBSchema)
        fixed_count += count

    if auth_errors:
        print(f"   → Repairing Auth Schema ({len(auth_errors)} errors)...")
        auth_schema, count = _repair_layer("Auth Schema", json.dumps(auth_schema.model_dump(), indent=2), auth_errors, AuthSchema)
        fixed_count += count

    if biz_errors:
        print(f"   → Repairing Business Logic ({len(biz_errors)} errors)...")
        business_logic, count = _repair_layer("Business Logic", json.dumps(business_logic.model_dump(), indent=2), biz_errors, BusinessLogic)
        fixed_count += count
...
def _repair_layer(layer_name: str, schema_json: str, errors: List[ValidationError], schema_class):
    error_descriptions = "\n".join([f"- [{e.error_type}] {e.description}" for e in errors])
