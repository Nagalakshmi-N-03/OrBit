...
def repair_schemas(
    ...
    if api_errors:
        ...
        api_schema, count = _repair_layer(
            "API Schema",
            json.dumps(api_schema.model_dump(), indent=2),
            api_errors,
            APISchema
        )
        fixed_count += count
    ...
    if db_errors:
        ...
        db_schema, count = _repair_layer(
            "DB Schema",
            json.dumps(db_schema.model_dump(), indent=2),
            db_errors,
            DBSchema
        )
        fixed_count += count
    ...
    if auth_errors:
        ...
        auth_schema, count = _repair_layer(
            "Auth Schema",
            json.dumps(auth_schema.model_dump(), indent=2),
            auth_errors,
            AuthSchema
        )
        fixed_count += count
    ...
    if biz_errors:
        ...
        business_logic, count = _repair_layer(
            "Business Logic",
            json.dumps(business_logic.model_dump(), indent=2),
            biz_errors,
            BusinessLogic
        )
        fixed_count += count
...
def _repair_layer(
    ...
    """
    Repairs a single schema layer
    Returns repaired schema + count of fixes
    """
    error_descriptions = "\n".join([
